"""Permutation-robust class-memory few-shot model."""
...
from net.heads.class_memory_head import PermutationAwareClassReadout
from net.heads.distribution_alignment_head import AuxiliarySWConsistencyHead
...
from net.ssm.permutation_memory import PermutationConsistentMemoryEncoder, SupportPermutationSampler


class PermutationRobustClassMemoryMambaNet(BaseConv64FewShotModel):
    """Multi-permutation class-memory scanning to reduce support-order sensitivity."""
    ...
    def __init__(
        ...
        num_support_permutations: int = 3,
        permutation_consistency_weight: float = 0.1,
    ) -> None:
        super().__init__(
        ...
        self.permutation_consistency_weight = permutation_consistency_weight
        ...
        self.memory_encoder = PermutationConsistentMemoryEncoder(
            memory_writer=ClassMemoryWriteSSM(token_dim, ssm_state_dim, depth=ssm_depth),
            permutation_sampler=SupportPermutationSampler(num_permutations=num_support_permutations),
        )
        self.readout_head = PermutationAwareClassReadout(token_dim, temperature=temperature)
        self.support_token_adapter = nn.Linear(token_dim, token_dim)
        self.support_token_norm = nn.LayerNorm(token_dim)
        self.sw_head = AuxiliarySWConsistencyHead(
    ...
    def forward(
        ...
        for batch_idx in range(bsz):
            ...
            for class_idx in range(s_tokens.shape[0]):
                class_memory, dispersion, _ = self.memory_encoder(
                ...
                consistency_bias.append(-self.permutation_consistency_weight * dispersion)
            ...
            consistency_bias_t = torch.stack(consistency_bias, dim=0)
            ...
            memory_logits = self.readout_head(
                query_readouts_t,
                class_memories_t,
                consistency_bias=consistency_bias_t,
            )
            logits = memory_logits
            sw_logits = torch.zeros_like(memory_logits)
            if self.use_sw:
                sw_logits = self.sw_head(refined_queries_t, conditioned_support_t)
            ...
            aux_payload["consistency_bias"].append(consistency_bias_t.detach().unsqueeze(0))

        scores = torch.cat(all_scores, dim=0)
        if not return_aux:
            return scores
        aux = {
            "memory_logits": torch.cat(aux_payload["memory_logits"], dim=0),
            "sw_logits": torch.cat(aux_payload["sw_logits"], dim=0),
            "consistency_bias": torch.cat(aux_payload["consistency_bias"], dim=0),
        }
        return scores, aux
