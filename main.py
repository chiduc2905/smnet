...
def train_loop(net, train_loader, val_X, val_y, args):
    ...
    Uses Top-K Model Averaging: maintains top K checkpoints based on validation
    accuracy and averages their weights to produce a more stable final model.
    """
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    ...
    for epoch in range(1, args.num_epochs + 1):
        ...
        # Update best model (save immediately when new best found)
